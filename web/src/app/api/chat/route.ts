...
const SYSTEM_PROMPT = `당신은 고려대학교 교육대학원 입학 및 학사 안내 도우미입니다.
아래 제공된 Q&A 게시판 답변과 공식 PDF 문서(모집요강, OT 학사안내, 입시 FAQ, 신입생 오리엔테이션 책자)를 기반으로 답변합니다.
...
7. 질문이 "입학 지원 요건"과 "입학 후 이수 요건" 모두에 해당될 수 있는 경우, 두 가지 관점을 모두 안내합니다. 예를 들어 "교원자격증 취득을 위해 전공을 몇 학점 들어야 하나요?"라는 질문에는 입학 시 학부 전공 36학점 이수 요건뿐 아니라, 입학 후 대학원에서 전공 50학점+교직 24학점 등 실제 자격증 취득 요건도 함께 안내해야 합니다.
8. 공식 PDF 문서의 내용을 Q&A 게시판보다 우선하여 참고합니다. PDF 문서가 더 정확하고 최신 정보입니다.`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 관련 Q&A 검색
    const relatedQAs = searchQA(message, 15);
    ...
    const chat = model.startChat({
      history: [
        ...
        },
        {
          ...
          parts: [
            {
              text: "네, 고려대학교 교육대학원 입학 및 학사 안내 도우미로서 Q&A 게시판과 공식 PDF 문서를 기반으로 정확하게 답변하겠습니다. 질문의 맥락에 따라 입학 요건과 입학 후 이수 요건을 구분하여 안내하겠습니다.",
            },
          ],
        },
        ...chatHistory,
